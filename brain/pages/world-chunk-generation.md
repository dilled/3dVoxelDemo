...
- **Chunks:** 16×16 blocks (384 m); tier-aware keep-set radius around the player chunk (M14.1 `WORLD.setTierLod()`, driven by the `TIER` seam [[m141-perf-monitor-quality-tiers]]): Chebyshev R=2 at HIGH/MED ⇒ 5×5 = 25 chunks, R=1 at LOW ⇒ 3×3 = 9; on-demand build / unbuild-behind with a buffer pool (retired chunks are pooled, never disposed); per-chunk budget = building occupancy by LOD ring [0.92, 0.85, 0.78].
...
- time: 2026-09-20T02:12:25
  kind: decision
  summary: "Keep-set radius is now tier-aware (M14.1): R=2 (5×5 = 25 chunks) at HIGH/MED, R=1 (3×3 = 9) at LOW, via WORLD.setTierLod() on the TIER seam ([[m141-perf-monitor-quality-tiers]])"
