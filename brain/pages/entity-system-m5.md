...
- time: 2026-09-17T19:30:07
  kind: note
  summary: "M9.2 landed the wake state machine on ENTITY (the M9-ready rig parts are now consumed): state/stateT on ENTITY, wake() trigger, per-state hooks + onAwaken, wakeP pose drives head/jaw/rings; ring spin moved from absolute t·spin to integrated _ringAng (continuous speed-up). See m92-wake-state-machine."
  source: M9.2 implementation
  affects: [entity-system-m5, m92-wake-state-machine]
